...
def make_arrival(callsign, departure, destination, rwy):
    ...
    Returns:
        str: The flight string to be feed into the scenario file.
    """
    flight_plan_route, position, level, fp_direction = choice(
        AIRPORT_SETTINGS[destination]['ARRIVAL_FPL'][departure]
        )
    (route,
     position_coords,
     expected_alt) = AIRPORT_SETTINGS[destination]['ARRIVAL_ROUTES'][rwy][position]
    random_altitude = choice(AIRPORT_SETTINGS[destination]['GET_FL'][fp_direction][level])
    flight_plan = '''
:*A:I:B738:364:{departure}:0000:0000:{random_altitude}:{destination}:00:00:0:0::/v/:
'''.format(
    departure=departure,
    random_altitude=random_altitude,
    destination=destination
